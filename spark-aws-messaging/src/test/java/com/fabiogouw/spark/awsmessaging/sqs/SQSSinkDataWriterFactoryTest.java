...
class SQSSinkDataWriterFactoryTest {
    ...
    private SqsClient createMockSqsClient() {
    ...
    private SqsClientBuilder createMockSqsClientBuilder(SqsClient mockSqs) {

        when(mockSqs.getQueueUrl(any(GetQueueUrlRequest.class))).thenReturn(GetQueueUrlResponse.builder().queueUrl("http://q").build());
        SqsClientBuilder mockSqsClientBuilder = mock(SqsClientBuilder.class);
        when(mockSqsClientBuilder.build()).thenReturn(mockSqs);
        when(mockSqsClientBuilder.httpClientBuilder(any(SdkHttpClient.Builder.class))).thenReturn(mockSqsClientBuilder);
        when(mockSqsClientBuilder.credentialsProvider(any(AwsCredentialsProvider.class))).thenReturn(mockSqsClientBuilder);
        return mockSqsClientBuilder;
    }

    @Test
    void when_CustomEndpointIsNotProvided_should_CreateDataWriterWithOnlyRegionConfiguration() {
        // Arrange
        SqsClient mockSqs = createMockSqsClient();
        SqsClientBuilder mockSqsClientBuilder = createMockSqsClientBuilder(mockSqs);
        try (MockedStatic<SqsClient> utilities = Mockito.mockStatic(SqsClient.class)) {
            utilities.when(SqsClient::builder).thenReturn(mockSqsClientBuilder);
